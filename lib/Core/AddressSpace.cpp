...
bool AddressSpace::resolveOne(ExecutionState &state, TimingSolver *solver,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {
    ...
    while (oi != begin) {
      ...
      bool mayBeTrue;
      if (!solver->mayBeTrue(state, mo->getBoundsCheckPointer(address),
                             mayBeTrue))
        return false;
      if (mayBeTrue) {
        result = *oi;
        ...
        if (!solver->mustBeTrue(
                state, UgeExpr::create(address, mo->getBaseExpr()), mustBeTrue))
          return false;
        if (mustBeTrue)
          break;
      }
    ...
    for (oi = start; oi != end; ++oi) {
      ...
      bool mustBeTrue;
      if (!solver->mustBeTrue(
              state, UltExpr::create(address, mo->getBaseExpr()), mustBeTrue))
        return false;
      if (mustBeTrue) {
        break;
      } else {
...
void AddressSpace::copyOutConcretes() {
  for (MemoryMap::iterator it = objects.begin(), ie = objects.end(); it != ie;
    ...
    if (!mo->isUserSpecified) {
      ...
      auto address = reinterpret_cast<uint8_t *>(mo->address);
...
bool AddressSpace::copyInConcrete(const MemoryObject *mo, const ObjectState *os,
  ...
  auto address = reinterpret_cast<uint8_t *>(src_address);
